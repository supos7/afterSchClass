...
logfile = 'addDelFreePass.log'

FORMAT = "%(asctime)-15s  %(levelname)s %(message)s"
logging.basicConfig(filename=logfile, level=logging.DEBUG, format=FORMAT)

logging.info('<<<<< The log file of addDelFreePass.exe >>>>>')

def usage():
   print("Usage: addDelFreePass -y year -m month -f file")
      ...
      if bBreak:
         ...
         for row in sheet.rows:
            if row[colFirst].row < rowFirst:
               ...
               continue
            # student
            if row[colFirst].value and row[colFirst +1].value and row[colFirst +2].value and row[colFirst +3].value and row[colFirst +4].value:
               ...
               if r is not None: # update code
                  stuId = r[0]
                  if -1 < row[colFirst +4].value.find(u'N'):
                     t = ('FPN',stuId)
                     cur.execute("UPDATE student SET code=? WHERE id=?", t)
                     t = ('FPN',stuId,cYear,cMonth)
                     cur.execute("UPDATE classStu SET code=? WHERE stuId=? AND classId IN (SELECT id FROM afterSchoolClass WHERE year=? AND month<?)", t)
                     logging.info('The student updated as a free pass: %s,%s,%s,%s,%s', stuId,stuGrade,stuClass,row[colFirst +2].value,row[colFirst +3].value)
                     ...
                     t = (stuId,cYear,cMonth)
                     cur.execute("SELECT id FROM classStu WHERE stuId=? AND classId IN (SELECT id FROM afterSchoolClass WHERE year=? AND month<?)", t)
                     for r in cur:
                        t = (r[0],)
                        cur1.execute("UPDATE classStu SET tuit_pay='Y',mcos_pay='Y' WHERE id=?", t)
                  elif -1 < row[colFirst +4].value.find(u'Q'):
                     t = ('FPQ',stuId)
                     cur.execute("UPDATE student SET code=? WHERE id=?", t)
                     t = ('FPQ',stuId,cYear,cMonth)
                     cur.execute("UPDATE classStu SET code=? WHERE stuId=? AND classId IN (SELECT id FROM afterSchoolClass WHERE year=? AND month<?)", t)
                     logging.info('The student updated as not free pass: %s,%s,%s,%s,%s', stuId,stuGrade,stuClass,row[colFirst +2].value,row[colFirst +3].value)

               else: # student not found
                  logging.info('The student not found: %s,%s,%s,%s', stuGrade,stuClass,row[colFirst +2].value,row[colFirst +3].value)
            else:
...
logging.info("Adding and deleting afterSchool free pass done.")
